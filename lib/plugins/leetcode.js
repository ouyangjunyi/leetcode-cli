  ...
  const getCategory = function (category, queue, cb) {
    plugin.getCategoryProblems(category, function (e, _problems) {
      if (e) {
        ...
        log.debug(
          category + ": getProblems got " + _problems.length + " problems"
        );
...
plugin.getProblem = function (problem, needTranslation, cb) {
  ...
  request.post(opts, function (e, resp, body) {
    ...
    problem.desc =
      q.translatedContent && needTranslation ? q.translatedContent : q.content;
...
plugin.getSubmissions = async (problem, cb) => {
...
plugin.signin = function (user, cb) {
  ...
  const spin = isCN
    ? h.spin("Signing in leetcode-cn.com")
    : h.spin("Signing in leetcode.com");
