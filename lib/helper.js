...
function getUnit(units, v) {
  for (let i = 0; i < units.length; ++i) {
    if (units[i].count <= 0 || v < units[i].count) return [v, units[i]];
    v /= units[i].count;
  }
}
...
h.prettyState = function (state) {
  switch (state) {
    case "ac":
      return this.prettyText("", true);
    case "notac":
      return this.prettyText("", false);
    default:
      return " ";
  }
};
...
h.prettyText = function (text, yesNo) {
  ...
  switch (yesNo) {
    case true:
      return chalk.green(icon.yes + text);
    case false:
      return chalk.red(icon.no + text);
    default:
      return text;
  }
};
...
h.langToExt = function (lang) {
  const res = LANGS.find(
    (x) => x.lang === lang || (x.lang == "cpp" && lang == "C++")
  );
...
h.badge = function (s, color) {
  s = " " + s + " ";
  if (color === "random") color = _.chain(COLORS).keys().sample().value();
